  ...
  const verificationDetail = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/verify/detail/${id}`
      );
  ...
  return (
    <div className={styles.verifyDetailContainer}>
      ...
      <div className={styles.detailCard}>
        ...
        <p>
          <strong>Verify Status:</strong> {detail.verifyStatus}
        </p>
        <p>
          <strong>Admin ID:</strong> {detail.adminId || "Not assigned"}
        </p>
        <p>
          <strong>Host ID:</strong> {detail.hostId}
        </p>
        <p>
      ...
      <div className={styles.hostInfo}>
        <div className={styles.hostCard}>
          ...
          <p>
            <strong>Host Name:</strong> {host.name}
          </p>
          <p>
            <strong>Host Type:</strong> {host.type}
          </p>
          <p>
            <strong>Host Address:</strong> {host.address}
          </p>
          <p>
            <strong>Host Description:</strong> {host.description}
          </p>
          <div className={styles.coordinates}>
            <p>
              <strong>Latitude:</strong> {host.lat}
            </p>
            <p>
              <strong>Longitude:</strong> {host.long}
            </p>
          </div>
        </div>
      </div>
        ...
        {host.photosHost && host.photosHost.length > 0 ? (
          host.photosHost.map((photo) => (
            <div key={photo.id} className={styles.photoItem}>
              ...
              <p>
                <strong>Photo Created At:</strong>{" "}
